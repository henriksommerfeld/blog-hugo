...
const theme = {
  ...
  applyTheme: function (theme) {
    ...
    console.log(message)
    document.getElementById('theme-switcher-indicator').checked = !isDark
  ...
  listenForExternalChange: function () {
    try {
      ...
      window.addEventListener('storage', () => {
        ...
        if (theme !== this.options.DARK) theme = this.options.LIGHT
        this.applyTheme(theme)
      })
    } catch (error) {
      console.warn('Not listening to theme change events', error)
    }
  },
  onLoad: function () {
...
const lightbox = {
  ...
  open: function (event, xRefs) {
    ...
    if (!img || img.getAttribute('src') !== imageUrl) {
      img && img.remove()
      setTimeout(() => {
        if (xRefs.lightbox.querySelectorAll('img').length === 0) this.showLoading = true
      }, 300)
...
const search = {
  ...
  hits: [],
  getDialog: function () {
    return document.getElementById('searchbox-container')
  },
  ...
  fetchIndex: function () {
    return fetch('/search-index.json').then(this.handleFetchResponse).catch(this.handleFetchResponse)
  },
  ...
  searchBoxChanged: function (phrase) {
    ...
    if (trimmedPhrase.length < 2 && trimmedPhrase !== '*') return
...
const menu = {
  ...
  isOpening: function () {
    return this.state === this.states.OPENING
  },
  isClosing: function () {
    return this.state === this.states.CLOSING
  },
  isClosed: function () {
  ...
  close: function () {
    if (this.state === this.states.CLOSED || this.state === this.states.CLOSING) return
  ...
  open: function () {
    if (this.state === this.states.OPEN || this.state === this.states.OPENING) return
...
const code = {
  ...
  isVisible: false,
  getDialog: function () {
    return document.getElementById('code-container')
  },
  close: function () {
    if (this.isVisible) {
