...
context('Navigation', () => {
  ...
  describe('Desktop', ()=> {
    ...
    it('Should go to next page', () => {
      cy.get('.paging-controls').findByText('Older').click({force: true})
      .url().should('equal', Cypress.config().baseUrl + '/page/2/');
    })
    
    it('Should go to next page', () => {
      cy.get('.paging-controls').findByText('Older').click({force: true})
      .url().should('equal', Cypress.config().baseUrl + '/page/3/');
    })
    
    it('Should go to previous page', () => {
      cy.visit('/page/4')
      cy.get('.paging-controls').findByText('Newer').click({force: true})
      .url().should('equal', Cypress.config().baseUrl + '/page/3/');
    })
    
    it('Should go to first page', () => {
      cy.get('.paging-controls').findByText('1').click({force: true})
      .url().should('equal', Cypress.config().baseUrl + '/');
    })
    
    it('Should go to first tag', () => {
      cy.get('.tag-cloud-tags a:first').click({force: true})
      .url().should('contain', Cypress.config().baseUrl + '/tags/');
      cy.findByText('Tag:').should('be.visible');
    })
    
    it('Should go to category Coding', () => {
      cy.get('.all-categories').findByText('coding').click({force: true})
      .url().should('equal', Cypress.config().baseUrl + '/categories/coding/');
    ...
    it('Should go to top of page', () => {
      cy.scrollTo('bottom').get('#page-footer').findByText('To top').should('be.visible').click({force: true});
