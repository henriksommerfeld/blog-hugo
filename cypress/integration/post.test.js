...
context('Post', () => {
  ...
  it('Should have publishing date', () => {
    cy.get('article time.published')
      ...
      .and('include', '2018-07-31 11:14:58 +0200')
  })
  ...
  it('Should close image lightbox on click', () => {
    cy.get('#lightbox img').click().should('not.have.class', 'open')
  })
  ...
  it('Should show tag Networking', () => {
    cy.get('article footer').within(() => {
      cy.findByText('Networking').should('be.visible').should('have.attr', 'href', '/tags/networking')
    })
  })

  it('Should show category Tooling', () => {
    cy.get('article footer').within(() => {
      cy.findByText('Tooling').should('be.visible').should('have.attr', 'href', '/categories/tooling')
    })
  })
  ...
  it('Should load comments on button click', () => {
    ...
    cy.findByText('Show Comments').click({ force: true }).get('#disqus_thread').should('be.visible')
  })
})
